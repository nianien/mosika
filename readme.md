...
- **往返稳定**：虚拟根和 `∅` 使单分支与多分支使用相同结构，保证“规则 → UI → JSON → UI → 规则”的语义一致性。
...
UI 数据模型在 `RNode` 上提供可选的 `name` 字段，`LNode`、`HNode` 通过继承获得同一字段；名称只用于 UI 展示，不参与 `ruleExpr()` 和规则求值。`UINodeAdapter.fromRule()` 将单一条件和复合条件统一规范化为 `JNode`，其 `rule` 分别指向 `RNode` 或 `LNode/HNode`。旧 JSON 中的 `CNode` 仍可反序列化并转换为规则，但经过“规则 → UI”规范化后会得到 `JNode(RNode)`。
