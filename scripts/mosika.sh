...
# Mosika 规则编排服务管理脚本。
#
# 用法：./scripts/mosika.sh <build|start|stop|restart|status|logs|dev|doctor|help>
#
set -euo pipefail
...
PORT="${MOSIKA_PORT:-8080}"
ADDRESS="${MOSIKA_SERVER_ADDRESS:-127.0.0.1}"
DB_PATH="${MOSIKA_DB_PATH:-$PROJECT_DIR/mosika-web/data/mosika.db}"
RUNTIME_BASE="${TMPDIR:-/tmp}"
RUN_DIR="${MOSIKA_RUN_DIR:-${RUNTIME_BASE%/}/mosika}"
STATE_FILE="$RUN_DIR/instance.state"
DEFAULT_LOG_FILE="$RUN_DIR/mosika.log"
LEGACY_RUN_DIR="$PROJECT_DIR/.run"
START_TIMEOUT="${MOSIKA_START_TIMEOUT:-60}"
STOP_TIMEOUT="${MOSIKA_STOP_TIMEOUT:-15}"
...
validate_config() {
    if ! is_positive_integer "$PORT" || [ "$PORT" -gt 65535 ]; then
        err "MOSIKA_PORT 必须是 1 到 65535 之间的整数，当前值：$PORT"
        return 1
    fi
    if ! is_positive_integer "$START_TIMEOUT" || ! is_positive_integer "$STOP_TIMEOUT"; then
        err "MOSIKA_START_TIMEOUT 和 MOSIKA_STOP_TIMEOUT 必须是正整数"
        return 1
    fi
...
adopt_legacy_state() {
    ...
    [ -z "${MOSIKA_RUN_DIR:-}" ] || return 1
    local legacy_pid_file="$LEGACY_RUN_DIR/mosika.pid"
    local legacy_log_file="$LEGACY_RUN_DIR/mosika.log"
    ...
    local legacy_jar=""
    local old_ui_jar="$PROJECT_DIR/mosika-ui/target/mosika-ui.jar"
...
cmd_start() {
    ...
    local java_options=(
        ...
        "-Dserver.port=$PORT"
        "-Dmosika.db.path=$DB_PATH"
    )
    if [ -n "${MOSIKA_JAVA_OPTS:-}" ]; then
        local extra_java_options=()
        read -r -a extra_java_options <<< "$MOSIKA_JAVA_OPTS"
...
cmd_dev() {
    ...
    JAVA_HOME="$JAVA_HOME" exec "$MAVEN_BIN" -pl "$MODULE" \
        ...
        -Dspring-boot.run.jvmArguments="-Dserver.address=$ADDRESS -Dserver.port=$PORT -Dmosika.db.path=$DB_PATH"
}
...
环境变量：
  MOSIKA_PORT           新实例端口，默认 8080
  MOSIKA_SERVER_ADDRESS 监听地址，默认 127.0.0.1
  MOSIKA_DB_PATH        SQLite 路径，默认 mosika-web/data/mosika.db
  MOSIKA_RUN_DIR        状态与日志目录，默认 \${TMPDIR:-/tmp}/mosika
  MOSIKA_START_TIMEOUT  启动等待秒数，默认 60
  MOSIKA_STOP_TIMEOUT   停止等待秒数，默认 15
  MOSIKA_JAVA_OPTS      额外 JVM 参数
EOF
}
